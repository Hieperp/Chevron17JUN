...
namespace TotalSmartCoding.CommonLibraries.BP
{
    ...
    public class ADODatabase
    {
        ...
        {
            return ADODatabase.ExecuteNonQuery(commandText, CommandType.Text, connectionString, null);
        }

        public static int ExecuteNonQuery(string commandText, CommandType commandType, string connectionString, IDataParameter[] dataParameter)
        {
            return ADODatabase.ExecuteNonQuery(commandText, CommandType.Text, connectionString, dataParameter, -1);
        }

        public static int ExecuteNonQuery(string commandText, CommandType commandType, string connectionString, IDataParameter[] dataParameter, int commandTimeout)
        {
            Console.WriteLine(commandText);
            using (SqlConnection sqlConnection = new SqlConnection(connectionString.Trim() == "" ? GlobalMsADO.ConnectionString() : connectionString.Trim()))
            {
                try
                {
                    sqlConnection.Open();
