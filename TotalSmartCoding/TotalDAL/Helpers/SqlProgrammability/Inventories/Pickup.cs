...
namespace TotalDAL.Helpers.SqlProgrammability.Inventories
{
    public class Pickup
    {
        ...
        private string BuildSQLNew(bool isPalletIDs)
        {
            ...
            queryString = queryString + "                   INNER JOIN Commodities ON ROUND(Pallets.Quantity - Pallets.QuantityPickup, " + (int)GlobalEnums.rndQuantity + ") > 0 AND Pallets.CommodityID = Commodities.CommodityID " + (isPalletIDs ? " AND Pallets.PalletID NOT IN (SELECT Id FROM dbo.SplitToIntList (@PalletIDs))" : "") + "\r\n";
